...
one rolegroup from a set of valid rolegroups
...
Schema
  ...
  One reference to a RoleGroup
  Embeds one RoleGroup (document store)



Field stored in the datastore for Admin and Single strategies

trolegroup

The field is named trolegroup, in order not to conflict with the method #rolegroup used in the role group DSL.
...
<pre>
User
 include Troles::GroupAdapter::RefOne
