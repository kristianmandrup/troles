...
The Api is now mostly implemented at least for some strategie (see fx bit_many)! 

We need some work on the operations object API and also tighten up the main API, make sure the right methods are public, are accessible as instance or class methods as they should be etc. Also test corner cases for API (such as nil arguments, strings etc.) and ensure an ArgumentError is raised on invalid input! 
...
There is now support for Caching and invalidation of the role_list when the roles change. Needs more testing...
...
<pre>
module Trole::Marshaller
  class MyCustomMarshaller < GenericMarshaller
