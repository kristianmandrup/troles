...
Note: #set_role is only required for :single role strategies. In this case #set_roles impl. is not required, as the default impl. proxies #set_role
...
To add a new strategy, you CAN create a special Strategy module that acts as the module included by the troles macro.
Using standard naming convention it will be found and used by the _troles_strategy_ macro method :)  
If you don't create such a Strategy module, troles will just use the BaseOne or BaseMany for the given namespace.

Using naming conventions, the strategy will always try to use a storage class of the form: _Troles::Storage::EncryptedStringMany_ 
in the example below (substitute Strategy for Storage).
...
Note: You only need to implement this if you want to use it as a hook to add some extra strategy implementation code, such as custom modules etc. 

An adapter usuaully requires a custom implementation of the Config class. Look at the _troles/common/config.rb_ to see what functionality is available that you can use.
...
As you can see, you have several nice convenience methods available to help set up these somewhat complex model relationships! (see _Troles::Common::Config_)
...
There will soon be a global ORM configuration options for troles, so you can do sth. like:

@Troles::Common::Config.default_orm = :mongoid@

And troles will use this settingin case you don't specify the orm when you configure a troles strategy.
