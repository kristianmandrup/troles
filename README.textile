...
"Screencast: run-down of this README":http://screencast.com/t/Yi0TrYz2

Note: There have been a few updates since this screencast ;)

h2. Status June 1, 2011

The Api is now partly implemented, see the specs! We need some work on the operations object API and also tighten up the main API, make sure the right methods are public, are accessible as instance or class methods as they should be etc.
Also test corner cases for API (such as nil arguments, strings etc.) and ensure an ArgumentError is raised on invalid input! 

When both Trole and Troles have stabilized and DRYed up their implementation, it might make sense to split the project into two.
For now, it makes sense to keep them in one project as to better find reusability ;) 
...
The Store API is the only API that could require override for a specific data store.

h2. Storage

Storage modules for different types of data stores can be found in the /storage folder
...
module Troles::Api::Core  
  ...
  # should return a Troles::Operations object
  def roles
...
h2. Creating DataStore Adapter

To create a DataStore Adapter, simply implement each of the strategies to be supported as shown here:
...
For Active Record the is no need for change, as an AR Role model implements the _where_ API used in the deault _#find_roles_ implementation of the generic _RefMany_ strategy, that returns an array of Role models.

However for Mongoid fx, there is a need to use _#all_ on the where result: @Role.where(:name => roles).all@
...
module Troles
  module Strategy
    module RefMany
      ...
      def strategy
        @strategy ||= MongoidStorage.new self
      end

      class MongoidStorage < Troles::Strategy::RefMany::Storage
...
You can also use a custom Calculator to store the roles in a non-standard format. This is used for the BitMany strategy, which 
has a special _Calculator::BitMask_ class which handles conversion between a list of symbols to an Integer bitmap representing it (relative to a valid roles list!). You can create your own Calculator, fx to Encrypt the roles info or whatever you like!
