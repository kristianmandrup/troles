...
module Troles::Mongoid::Api
  module Config
    def troles_config
      @troles_config ||= Troles::Mongoid::Config.new self, :generic => false
    end      
  end  
end  
</pre>
...
Then call it like this from the strategy module: @configure_role_field(:strategy => :string_one)@ 
...
Note: This config pattern needs more consideration and is bound to change and be improved!
To use your adapter, simply pass an extra option to the troles_strategy macro:

<pre>
  User.troles_strategy :bit_one, :orm => :mongoid
</pre> 
...
<pre>
  User.troles_strategy :bit_one, :orm => :mongoid, :auto_load => true
</pre> 

Now there is also support for specifying config options directly via the macro:

<pre>
  User.troles_strategy :bit_one, :orm => :active_record, :auto_load => true, :role_join_model => 'UserRole'
</pre> 
...
<pre>
User.troles_strategy :bit_one do |config|
  config.orm = :active_record
  config.role_join_model = 'UserRole'
...
<pre>
User.troles_strategy :bit_one do
  orm = :active_record
  role_join_model = 'UserRole'
  auto_load = false
end.configure! 
</pre>
